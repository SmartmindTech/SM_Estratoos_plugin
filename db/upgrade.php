...
function xmldb_local_sm_estratoos_plugin_upgrade($oldversion) {
    ...
    // v1.8.0: Add get_activity_progress function for lightweight activity metadata retrieval.
    // Also fixes SCORM currentslide extraction from lesson_location tracking data.
    if ($oldversion < 2025012600) {
        require_once(__DIR__ . '/install.php');

        // Add the new get_activity_progress function to the service.
        xmldb_local_sm_estratoos_plugin_add_to_mobile_service();
    ...
    // v2.1.24: Add self-service profile update function (update_user).
    if ($oldversion < 2025020324) {
        require_once(__DIR__ . '/install.php');

        // Rebuild service functions to include the new update_user API.
        xmldb_local_sm_estratoos_plugin_add_to_mobile_service();

        purge_all_caches();
        upgrade_plugin_savepoint(true, 2025020324, 'local', 'sm_estratoos_plugin');
    }

    // v2.1.29: Add calendar event update function.
    if ($oldversion < 2025020329) {
        require_once(__DIR__ . '/install.php');

        // Rebuild service functions to include the new update_calendar_event API.
        xmldb_local_sm_estratoos_plugin_add_to_mobile_service();
